...
function highlightHTMLContent(htmlContent, plainText, plainTextPositions) {
  ...
  for (const position of plainTextPositions) {
      ...
      const startIndex = htmlContent.indexOf(textToHighlight, currentPosition);

      if (startIndex === -1) {
          highlightedHTML += htmlContent.substring(currentPosition);
          break;
      }

      highlightedHTML += htmlContent.substring(currentPosition, startIndex);
      highlightedHTML += startTag + textToHighlight + endTag;

      currentPosition = startIndex + textToHighlight.length;
  }
